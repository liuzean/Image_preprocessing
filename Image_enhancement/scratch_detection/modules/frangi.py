...
def main() -> None:
    ...
    frangi_config = FrangiConfig(
        enabled=True,
        sigmas=(2.0,2.5),   #表示检测尺度,尺度越多，适应的划痕宽度越广,sigmas越大，检测的划痕越宽，sigmas越小，检测的划痕越窄或者越浅
        alpha=0.5,        #用于三维图像
        beta=0.3,         #控制“线状结构”和“圆形/块状结构”的区分强度。越小倾向于细长线状，越大，限制越小，弱划痕更容易保留，但纹理和斑点，圆形/块状结构也可能增加。
