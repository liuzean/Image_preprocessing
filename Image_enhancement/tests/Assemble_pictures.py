...
ROOT_DIR = Path(r"E:\projects\datasets\Power_box\old\background_results")
LEFT_DIR_NAME = "007"
RIGHT_DIR_NAME = "010"
OUTPUT_DIR_NAME = "Assemble_results"
NAME_KEYWORD = "background_corrected"
IMAGE_EXTENSIONS = {
    ".bmp",
    ".gif",
    ".jpeg",
...
def assemble_pair(left_path: Path, right_path: Path, output_path: Path) -> None:
    ...
    with Image.open(left_path) as left_source, Image.open(right_path) as right_source:
        # Respect the camera orientation metadata before calculating dimensions.
...
def assemble_matching_images(
    ...
    """Assemble same-name target images from the configured source folders."""
